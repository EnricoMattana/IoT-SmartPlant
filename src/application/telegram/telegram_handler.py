...
class TelegramWebhookHandler:
    # La base è stata presa dal template visto a lezione.
    ...
    def start(self):
        logging.info("[TelegramWebhook] Inizio setup Telegram Bot...")

        # Step 1: Avvio deell'event loop asincrono separato, necessario per il funzionamento di telegram. 
        self.loop = asyncio.new_event_loop()
        asyncio.set_event_loop(self.loop)

        # Step 2: Inizializzazione del Bot
        self.application = Application.builder().token(TELEGRAM_TOKEN).build()  # Creazione dell'istanza del bot
        self.application.loop = self.loop  
        self.app.config["TELEGRAM_BOT"] = self.application.bot # Salviamo il bot nelle config.


        # Step 3: Registra tutti gli handlers
        self.setup_handlers()

        # Step 4: Startiamo il bot
        self.loop.run_until_complete(self.application.initialize())
        self.loop.run_until_complete(self.application.start())

        # Registra gli endpoint flask per poter comunicare con telegram, come registriamo le routes alle API che scriviamo noi
        init_routes(self.application)
        self.app.register_blueprint(webhook)

        # Step 6: Setup ngrok per ottenere un URL pubblico a cui telegram può comunicare
        ...
        # Questo serve per gestire chiamate asincrone da altri thread, come per mandare notifiche per non bloccare l'app.
    ...
    def setup_handlers(self):
        # Elenco degli handler della conversazione:
    ...
    def stop(self):
        logging.info("[TelegramWebhook] Stop del Telegram bot...")
